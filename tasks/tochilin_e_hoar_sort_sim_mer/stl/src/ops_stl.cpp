...
int TochilinEHoarSortSimMerSTL::ResolvePartCount(std::size_t size) {
  ...
  const int concurrency = ResolveWorkerCount(size / kMinPartSize);
  const int max_parts_by_size = static_cast<int>(size / kMinPartSize);
  return std::max(1, std::min(concurrency, max_parts_by_size));
}

int TochilinEHoarSortSimMerSTL::ResolveWorkerCount(std::size_t task_count) {
  if (task_count == 0) {
    return 1;
  }

  const int available_threads = std::max(1, ppc::util::GetNumThreads());
  return std::max(1, std::min(available_threads, static_cast<int>(task_count)));
}
...
void TochilinEHoarSortSimMerSTL::SortParts(std::vector<int> &data, const std::vector<std::size_t> &boundaries) {
  ...
  const int worker_count = ResolveWorkerCount(static_cast<std::size_t>(part_count));
  ...
  for (int worker = 0; worker < worker_count; ++worker) {
    workers.emplace_back([&, worker] {
      const int begin_part = (worker * part_count) / worker_count;
      const int end_part = ((worker + 1) * part_count) / worker_count;

      for (int part = begin_part; part < end_part; ++part) {
        const std::size_t begin = boundaries[static_cast<std::size_t>(part)];
        const std::size_t end = boundaries[static_cast<std::size_t>(part) + 1];
        if (begin < end) {
          QuickSortSequential(data, static_cast<int>(begin), static_cast<int>(end - 1));
...
std::vector<std::size_t> TochilinEHoarSortSimMerSTL::MergePass(const std::vector<int> &src, std::vector<int> &dst,
  ...
  const int worker_count = ResolveWorkerCount(merge_pairs);

  if (worker_count == 1) {
    for (std::size_t pair_idx = 0; pair_idx < merge_pairs; ++pair_idx) {
    ...
    for (int worker = 0; worker < worker_count; ++worker) {
      workers.emplace_back([&, worker] {
        const std::size_t begin_pair = (static_cast<std::size_t>(worker) * merge_pairs) / worker_count;
        const std::size_t end_pair = (static_cast<std::size_t>(worker + 1) * merge_pairs) / worker_count;

        for (std::size_t pair_idx = begin_pair; pair_idx < end_pair; ++pair_idx) {
          const std::size_t left = current_boundaries[pair_idx * 2];
          const std::size_t mid = current_boundaries[(pair_idx * 2) + 1];
          const std::size_t right = current_boundaries[(pair_idx * 2) + 2];
          MergeRanges(src, dst, left, mid, right);
