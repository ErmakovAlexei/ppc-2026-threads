...
`TBB` показывает лучшую медиану среди thread-backend-ов. Это выглядит правдоподобно, потому что реализация сочетает
...
можно объяснить тем, что на больших входах динамическая балансировка диапазонов quicksort через oneTBB оказывается
...
speedup на основном размере `N = 2_000_000`. `OMP` и `STL` тоже дали устойчивое ускорение и выглядят методически
сильными реализациями той же идеи. Гибридная `ALL` после улучшения межпроцессного merge-этапа стала корректной и
