...
DenseMatrix MakeRandomDense(int n, double density) {
  DenseMatrix m(static_cast<std::size_t>(n),
                std::vector<std::complex<double>>(static_cast<std::size_t>(n), std::complex<double>(0.0, 0.0)));
  ...
  for (int i = 0; i < n; ++i) {
    for (int j = 0; j < n; ++j) {
      if (dis_prob(gen) < density) {
        m[static_cast<std::size_t>(i)][static_cast<std::size_t>(j)] = std::complex<double>(dis_val(gen), dis_val(gen));
      }
    }
  }
  return m;
...
MatrixCRS DenseToCRS(const DenseMatrix &m, double eps = 1e-12) {
  ...
  for (int i = 0; i < rows; ++i) {
    for (int j = 0; j < cols; ++j) {
      const auto v = m[static_cast<std::size_t>(i)][static_cast<std::size_t>(j)];
...
class ErmakovARunPerfTestSparMatMultTBB : public ppc::util::BaseRunPerfTests<InType, OutType> {
 protected:
...
TEST_P(ErmakovARunPerfTestSparMatMultTBB, RunPerfModes) {
...
const auto kAllPerfTasks =
    ppc::util::MakeAllPerfTasks<InType, ErmakovASparMatMultTBB>(PPC_SETTINGS_ermakov_a_spar_mat_mult_tbb);

const auto kGtestValues = ppc::util::TupleToGTestValues(kAllPerfTasks);

const auto kPerfTestName = ErmakovARunPerfTestSparMatMultTBB::CustomPerfTestName;

INSTANTIATE_TEST_SUITE_P(RunModeTests, ErmakovARunPerfTestSparMatMultTBB, kGtestValues, kPerfTestName);
